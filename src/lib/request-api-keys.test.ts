...
const AOMI_BACKEND_API_KEY_ENV = "NEXT_PUBLIC_AOMI_BACKEND_API_KEY";
...
test("request auth globals keep the env-configured Aomi key separate from the Para key", async () => {
  process.env[AOMI_BACKEND_API_KEY_ENV] = "aomi-from-env";
...
test("request auth globals reject a missing Aomi env key", async () => {
  delete process.env[AOMI_BACKEND_API_KEY_ENV];
  ...
  await assert.rejects(
    ...
    /NEXT_PUBLIC_AOMI_BACKEND_API_KEY is not defined/,
  );
});
...
test("request api key module uses a static NEXT_PUBLIC env reference for client bundling", async () => {
  ...
  assert.match(source, /process\.env\.NEXT_PUBLIC_AOMI_BACKEND_API_KEY/);
});
